...
const i18n = createI18n({
  ...
  fallbackLocale: import.meta.env.VITE_FALLBACK_LOCALE || 'zh-Hans',
  messages,
...
export function getTemplateByKey(key: string) {
  ...
  const fallbackLocale = (import.meta.env.VITE_FALLBACK_LOCALE as string) || 'zh-Hans'

  const message = messages[locale] || messages[fallbackLocale]
  if (!message) {
    console.warn(`[i18n] No messages found for locale: ${locale}`)
    return ''
  }
